// src/main/webapp/resources/js/attractions.js
(async function () {
  ...
  async function refreshFavs() {
    try {
      favs = await ETBACKEND.get('/scrap?action=list');
  ...
  async function addFav(item) {
    await ETBACKEND.post('/scrap?action=add', item);
    await refreshFavs();
  }
  async function removeFav(id) {
    await ETBACKEND.delete(`/scrap?action=remove&id=${encodeURIComponent(id)}`);
    await refreshFavs();
  }
  ...
  async function search() {
    try {
      ...
      console.error(e);
      listEl.innerHTML = `<div class="text-muted small">데이터를 불러오지 못했습니다.</div>`;
    }
  }
  ...
  function render(items) {
    ...
    items.forEach((it) => {
      ...
      scrapBtn.addEventListener('click', async (ev) => {
        ev.stopPropagation();
        try {
          if (isFav(id)) {
            await removeFav(id);
          ...
          if (err.status === 401) {
            ...
            console.error(err);
            ETUI.toast('스크랩 처리 중 오류가 발생했습니다.', 'danger');
          }
        }
      });
      ...
      if (
        ...
        const node = buildInfoWindow({ title, addr, img });
        kakao.maps.event.addListener(marker, 'click', () => openInfo(pos, node));
      }
    });
  ...
  function buildInfoWindow({ title, addr, img }) {
    ...
    node.innerHTML = `
      ...
      <div class="marker-img-wrap">
        <img src="${img || PLACEHOLDER}" alt="${title}" onerror="this.src='${PLACEHOLDER}'">
      </div>`;
